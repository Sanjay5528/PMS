...
export class DatasetComponent {
  ...
  parmsConvertIntoString(
    ...
    // if (DataType !== typeof defaultValue || isNaN(defaultValue)) {
    //   return this.dialogService.openSnackBar(`The Data Type Is Mismatched. Expected: ${DataType}, Received: ${typeof defaultValue}`, "OK");
    // }

    const FilterParams: any = this.dataSet.get("FilterParams")
      .controls as FormArray;

    let present_Value: any = FilterParams.at(index).get(keychange).value;
    let ChnagedParmsValue: any =
      "{{" + Param + "-" + present_Value.replace(Param, "") + "}}";
    this.parmsNotExist(
      this.dataSet.value["FilterParams"],
      keychange,
      ChnagedParmsValue
    ).then((Flag: any) => {
      if (!Flag)
      ...
      if (Flag == true) {
        ...
        FilterParams.at(index).get(keychange).setValue(ChnagedParmsValue);
  ...
  AggConvertIntoString(
    ...
    if (newColumnInsert == true) {
      ...
      if (!this.newcustomfield.includes(data)) {
  ...
  addFieldintoOptions(values: any) {
    let data = {
  ...
  savedataSet() {
    ...
    let dataset: any = this.dataSet.value;
    dataset.Filter = this.savegrp();
    if(isEmpty(dataset.SelectedList)){
this.dialogService.openSnackBar("Selected Column is Required","OK")
  ...
  getOperators(field: any, i: any, index: any) {
    ...
    } else if (field.type in this.NumberictypeMapping) {
      // Number
      // this.dateflag[i]=false
      this.operatorOptions[i][index] = [
        {
  ...
  removeField(i: number, index: any) {
    if (i == 0 && index == 0) { // todo isempty check
      this.grp = [];
      this.button_Flag = false;
    }else if (isEmpty(this.grp[i][index])) { // todo isempty check

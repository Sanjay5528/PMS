...
import { Injectable, Output, ViewChild, EventEmitter, TemplateRef } from '@angular/core';
...
console.log(ctrl.id ? 'Edit' : 'Add');
  ...
  async saveFormData(ctrl: any, file?: any): Promise<any> {
    ...
    if (!ctrl.form.valid) {
      ...
      if (data === 'INVALID') {
      // For other controls, collect their labels
     ...
     if(ctrl?.form?.data){
      ...
      if (ctrl.config.form.collectionName == "modules") {
        if (ctrl.autoGroupColumnDef?.headerName == "Parent Modules") {
          Object.assign(data, { parentmodulename: "", projectid: ctrl.response?.projectid || ctrl.grid.response.projectid })
        } else if (ctrl.aggrid.autoGroupColumnDef.headerName == "Parent Modules") {
          Object.assign(data, { projectid: ctrl.response?.projectid || ctrl.grid.response.projectid, parentmodulename: ctrl.deletedData.modulename })
        } else {
        ...
        debugger
        Object.assign(data,{projectid: ctrl.response?.projectid || ctrl.grid.response.projectid,})
      }
        ...
        if(ctrl.butText=="Update"){
          debugger
          Object.assign(data,{ moduleid: ctrl.model.moduleid, projectid: ctrl.model.projectid, projectname: ctrl.model.projectname })
        ...
        Object.assign(data, { moduleid: ctrl.deletedData.moduleid, projectid: ctrl.response?.projectid || ctrl.grid.response.projectid, projectname: ctrl.grid.response.projectname })
      console.log("lavanya",data);}
        ...
        Object.assign(data, { moduleid: ctrl.moduleid })
         ...
         Object.assign(data,{testcasename:sessionStorage.getItem('testcasename')})
         Object.assign(data,{projectname:sessionStorage.getItem('projectname')})
      }
     ...
     //while saving set default values
      ...
      if (ctrl.formAction == 'Add' || ctrl.config.formAction == 'Add') {
        ...
        this.dataService.saveAll('user',userdetails).subscribe((res: any) => {
          res
          // this.dialogService.openSnackBar("Data has been updated successfully", "OK")
        ...
        // }
          ...
          this.dataService.save(data, ctrl).pipe(
            catchError((error) => {
              if (error.status === 400) {
            ...
            if (error.status === 400) {
              ...
              this.dialogService.openSnackBar("ID is Already Exist, Create New", "OK");
            } else {
        ...
        debugger
        ctrl.model = data
        console.log(ctrl.collectionName=="employee"&&ctrl.formName=="employee");
        ...
        this.dataService.update(data, ctrl).subscribe((res: any) => {
          ...
          this.dialogService.openSnackBar("Data has been updated successfully", "OK")
          // resolve(res)
          this.dataService.UpdateAll('user',data.email,userdetails).subscribe((res: any) => {
            res
            this.dialogService.openSnackBar("Data has been updated successfully", "OK")
            resolve(res)
          })
